...
const cronScheduler = (context) => {
    const promise = new Promise(async(resolve, reject) => {
        try{
            ...
            context.log('Cron Scheduler is started')
            ...
            let result = await sql.execute(myDb, qs)
            let rows = result.recordset
            context.log('User rows length: '+ rows.length)
            if(!rows.length) throw new Error('No User available to update')
            for(let userId of rows)
            {
                context.log('data for loop starts: '+ userId)
                let xero = await xeroClient.initalize(userId)
                context.log('userId started: '+ userId)
                context.log('Xero is initialized')
                context.log('xero.MyTenants: '+ xero.MyTenants.length)
                for(let t of xero.MyTenants)
                {
                    let options = {
                        tenantId: t.TenantId,
                    ...
                    context.log(t.TenantId+' Tenant data is fetched.')
                    t.LastSyncDate = new Date()
                    await sql.update(process.env.DB_NAME, t)
                    context.log(t.TenantId+' Last sync is updated for Tenant.')
                }
            }
            
            await sql.closeAll()

            context.log('Cron Scheduler is completed successfully.')
            let res = {
                statusCode: 200,
                body: "Data fetched"
            }
